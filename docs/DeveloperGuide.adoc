...
From our verbal surveys with test users and user stories, we have decided to implement a Planning mode in order for users to plan their dives ahead of their trips.
...
** Method to implement: +
*Step 1:* Create a new Command [link to xxx]
+
*Step 2:* Create a enum with the parameters you want to sort the data by (E.g Time, Location and Duration)
+
*Step 3:* Ensure that the divelog refreshes / reflects the newly sorted

** Pros: To increase the number of parameters / methods to sort the data by, simply create a new corresponding comparator, add the respective prefixes the parse for, and to increase the number of parameters included in the Enum file.
** Cons: Can be troublesome to implement at first.
...
=== New Parameters & conditions in Dive Log
...
==== Time and OurDate
Time and OurDate are 2 of the new parameters in the Dive Log.
...
to the case of undo. For additions, additions are made to the model, which is eventually committed to storage.
