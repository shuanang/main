...
== Introduction
Divelog is a digital dive log for tracking dives that aims to simplify safe dive planning,logging and calculations.
...
Pressure groups are denoted by a letter between A to Z which marks the nitrogen
concentration in a diver's body (A Pressure group of "A" shows that the diver's body is almost free of nitrogen).
As of v1.4, the pressure group is automatically calculated within a dive, all one needs to enter is their
